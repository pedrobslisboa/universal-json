...
let count = function None -> 0 | Some { count; _ } -> count
let data = function None -> Sempty | Some { data; _ } -> data
...
let of_string s =
  ...
  from (fun _ ->
      (* We cannot use the index passed by the [from] function directly
         ...
         guarantee that it will start from 0. For example, in the case of
         [Stream.icons 'c' (Stream.from_string "ab")], the first access to
         the string will be made with count [1] already. *)
      let c = !count in
