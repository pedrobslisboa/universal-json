...
let with_default () =
  ...
  let a, b, c =
    Json.Decode.with_default (3, 2, 1)
...
let assoc () =
  ...
  match Json.Decode.assoc Json.Decode.string assocJson with
  | [ (_, name) ] ->
      Alcotest.(check @@ Alcotest.string) "should be equal" "John Doe" name
  | _ -> Alcotest.fail "Assoc don't match expected"
...
let map () =
  let intValue = (Json.Decode.map (( + ) 2)) Json.Decode.int (`Int 23) in
